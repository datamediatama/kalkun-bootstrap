...
$("a.global_delete").on('click', null, action_delete = function(){
...
/**
	...
	$("a.recover_button").on('click', null, action_recover = function()
	{
		...
		else 
		{
			...
			//alert(count + ' conversation recovered');
...
$(".move_to").on('click', null, function() {
var count = $("input:checkbox:checked").length;
if(count==0) { 
...
$(".move_to_button").on('click', null, message_move = function() {
...
$("span.message_toggle").on('click', null, function(){
...
$("a.select_all_button").on('click', null, select_all = function(){
...
$("a.clear_all_button").on('click', null, clear_all =  function(){
...
$("input.select_message").on('click', null, function()
{
...
$("a.refresh_button").on('click', null, refresh = function(type){
	if(type != 'retry') {
...
$('a.reply_button, a.forward_button').on('click', null, message_reply =  function() {
...
$("#compose_sms_container").load(url, { 'type': type, 'param1': param1, 'param2': param2}, function() {
  $("#maincontainer").css({"opacity":"0.7","z-index":"100"});
  $(this).dialog({
/*  	
    modal: true,
    draggable : true,    
...
*/
/*
modal:true, closeOnEscape: false, draggable:false, resizable:false, width: 550,
			close : function(){
                  $("#maincontainer").css({"opacity":"1","z-index":"100"});
              },	
...
//$("#compose_sms_container").dialog('open');
		$(document).ready(function() {
          $("#maincontainer").css({"opacity":"0.7","z-index":"100"});
          $("#compose_sms_container").dialog({modal:true, closeOnEscape: false, draggable:false, resizable:false, width: 550, open: function(event, ui) {$("#message").focus();}, close : function(){
                  $("#maincontainer").css({"opacity":"1","z-index":"100"});
              }  }, "modal", true).dialog("open");
        });
return false;
});    
...
$('a.detail_button').on('click', null, function() {
...
$('.add_to_pbk').on('click', null, function() {
var param1 = $(this).parents('div:eq(1)').children().children('input.item_number').val();  /* phone number */
...
// report spam
$(".spam_button").on('click', null, function() {
...
$(".ham_button").on('click', null, function() {
    var count = $("input:checkbox:checked:visible").length;
    if(count==0) { 
    	show_notification("<?php echo lang('tni_msg_no_conv_selected'); ?>");
    }
...
$(".resend_bulk").on('click', null, function() {
var count = $("input:checkbox:checked").length;
if(count==0) { 
	$('.notification_area').text("<?php echo lang('tni_msg_no_conv_selected'); ?>");
	$('.notification_area').show();
