...
$(document).ready(function() {
	...
	$('#compose_sms_normal').bind('click', compose_message = function() 
	{
		...
		$("#compose_sms_container").load('<?php echo site_url('messages/compose')?>', { 'type': "normal" }, function() {
		  $(this).dialog({
modal:true, closeOnEscape: false, draggable:false, resizable:false, width: 550, close : function(){
			...
			'<?php echo lang('tni_send_message'); ?>': function() {
				...
				$.post("<?php echo site_url('messages/compose_process') ?>", $("#composeForm").serialize(), function(data) {
					...
					setTimeout(function() {$("#compose_sms_container").dialog('destroy');
				$("#maincontainer").css({"opacity":"1","z-index":"100"});
				$("#compose_sms_container").hide();} , 1500);
				});
				}
			},
	...
	$('#addfolder').click(function() {	
		...
		height: 220,
		modal: true,
		closeOnEscape: false, draggable:false, resizable:false, close : function(){
                  $("#maincontainer").css({"opacity":"1","z-index":"100"})},
		buttons: {
			'Save': function() {
		...
		//$('#addfolderdialog').dialog().dialog('open');
		return false;
	});	
